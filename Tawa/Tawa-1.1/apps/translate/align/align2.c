...
int verbose = 0;                /* -v (verbose) arg */
int debug = 0;                  /* -b (debug) arg */
...
/*
  ...
  dist_funct(x1, y1, x2, y2) is a distance function of 4 args:
    ...
    dist_funct(x1, 0, 0, 01 gives cost of deletion of x1.
    dist_funct(0, y1, 0, 01 gives cost of insertion of y1.
    dist_funct(x1, y1, x2, 01 gives cost of contraction of (x1,x2) to y1.
     ...
     char **argv;
{
  ...
  /* parse arguments */
  while((c = getopt(argc, argv, "bd:D:v")) != EOF)
    switch(c) {
    case 'b':
      debug = 1;
      break;
  ...
  for( ; hard_regions1 < hard_end1 ; hard_regions1++, hard_regions2++) {
    ...
    if (debug)
      fprintf (stderr, "processing regions, lengths %d %d\n", *len1, *len2);
