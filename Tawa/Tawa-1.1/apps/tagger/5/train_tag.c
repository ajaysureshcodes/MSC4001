/* Trains on the input file of tags & words and writes out the model file. */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define PERFORMS_EXCLS TRUE                         /* Whether PPM Model performs exclusions */
...
{
    fprintf (stderr,
	     "Usage: train_tag [options] -i tagged-input-file -o model-output-file -t tagset-file\n"
    ...
    fprintf (stderr,
	     ...
	     "  -o fn\tmodel output filename=fn (required argument)\n"
	     ...
	     "  -2 n\tmax order of character model(s)=n (optional; default=4)\n"
...
init_arguments (int argc, char *argv[])
{
    ...
    Debug.level = 0;
    Debug.range = FALSE;
    while ((opt = getopt (argc, argv, "d:i:o:p:rt:T:1:2:3")) != -1)
	switch (opt)
	{
	case 'd':
...
/* Trains on the sequence of tags, words and characters in the text
   and writes out the model file. */
{
    unsigned int word, word_pos, tag, tag_id, tag_count, tag_word_symbol;
    unsigned int tag_context, tagset_table, tags_alphabet_size, tags_model;
	...
	*/
	...
	if (Debug.range)
	  {
	    fprintf (stderr, "Pos %d Updating model for word {", word_pos);
	...
	TLM_update_context (tag_model, tag_context, tag_word_symbol);
    }
...
main (int argc, char *argv[])
{
    ...
    /*TXT_dump_table (Stderr_File, tagset_table);*/
    ...
    process_file (Input_file, tag_model);

    TLM_write_model (Output_file, tag_model, TLM_Static);
