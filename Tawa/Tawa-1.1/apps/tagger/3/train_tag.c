...
{
    fprintf (stderr,
	     "Usage: train_tag [options] -i tagged-input-file -o output-files-prefix -t tagslist-file\n"
    ...
    fprintf (stderr,
	     ...
	     "  -o fn\toutput files filename prefix=fn (required argument)\n"
...
init_arguments (int argc, char *argv[])
{
    ...
    Debug.level = 0;
    Debug.range = FALSE;
    while ((opt = getopt (argc, argv, "d:i:o:p:t:T:1:2:")) != -1)
	switch (opt)
	{
	case 'd':
    ...
    if (!Tagslist_found || !Input_found || !Output_found || !Title_found)
      {
	usage ();
	exit (1);
      }
...
/* Encodes the tag, and word. */
{
	...
	/* Terminate the word with sentinel symbol as some "words" may have
	   nulls in them */
	TLM_update_context (chars_model, chars_context,
			    TXT_sentinel_symbol ());
      }
}
...
main (int argc, char *argv[])
{
    ...
    process_file (Input_file, tagslist_table, Tags_model, Words_model,
		  Chars_model);
