...
{
    fprintf (stderr,
	     "Usage: encode_tag [options] -i tagged-input-file -o encoded-output-file -t tagslist-file\n"
...
init_arguments (int argc, char *argv[])
{
    ...
    Debug.level = 0;
    Debug.range = FALSE;
    while ((opt = getopt (argc, argv, "Ad:i:o:p:rt:T:1:2:")) != -1)
	switch (opt)
	{
	case 'A':
	    Debug.coder = TRUE;
...
/* Encodes the tag, and word. */
{
    ...
    tag_id += IS_ASCII; /* Make sure tag id is not printable */
    if (Debug.range)
        fprintf (stderr, " tag id: %d\n", tag_id);
    TLM_encode_symbol (tags_model, tags_context, coder, tag_id);
	...
	TLM_encode_symbol (words_model, words_context, coder, word);
      }
	...
	while (TXT_get_symbol (word, pos, &symbol))
	  {
	    TLM_encode_symbol (chars_model, chars_context, coder, symbol);
	    pos++;
	  }
	/* Terminate the word with sentinel symbol as some "words" may have
	   nulls in them */
	TLM_encode_symbol (chars_model, chars_context, coder,
			   TXT_sentinel_symbol ());
      }
}
...
/* Encodes the sequence of tags, words and characters in the text. */
{
	...
	if (Debug.range)
	  {
	    fprintf (stderr, "Pos %d Encoding word {", word_pos);
	    TXT_dump_text (Stderr_File, word, TXT_dump_symbol);
	    fprintf (stderr, "} tag {");
    ...
    /* Encode a null tag to mark EOF */
    TLM_encode_symbol (tags_model, tags_context, coder, 0);
...
main (int argc, char *argv[])
{
    ...
    process_file (Input_file, tagslist_table, Tags_model, Words_model, Chars_model, Coder);
