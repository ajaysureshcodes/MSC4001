/* Corrects OCR text given a trained model. */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <assert.h>
...
boolean OCR_Viterbi = FALSE;
unsigned int OCR_stack_depth = 0;
...
main (int argc, char *argv[])
{
    ...
    if (OCR_Viterbi)
        ...
        transform_model = TTM_create_transform (TTM_Stack, TTM_Stack_type0, OCR_stack_depth, 0);

    TTM_add_transform (transform_model, 0.0, "%w", "%w"); /* add transform for every character first */

    /* Add OCR corrections here: */
    TTM_add_transform (transform_model, 0.0, "I", "l"); /* add transform for every character first */
