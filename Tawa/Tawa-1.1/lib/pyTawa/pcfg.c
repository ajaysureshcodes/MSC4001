...
#define PCFG_SYMBOL_TYPE 0      /* Specifies that the rule contains a
				   terminal symbol at this point. */
#define PCFG_NONTERMINAL_TYPE 1 /* Specifies that the rule contains a
				   non-terminal symbol at this point. */
#define PCFG_BOOLEAN_TYPE 3     /* Specifies that the rule contains a boolean
...
char **PCFG_labels = NULL;
unsigned int PCFG_labels_alloc = 0;

unsigned int Sentinel1_Symbol = NIL;

boolean
PCFG_get_rhs (unsigned int rhs, unsigned int pos, unsigned int *rhs_type,
	     unsigned int *rhs_symbol, unsigned int *new_pos)
/* Gets the type and symbol from the right hand side (rhs) of the PCFG rule. */
{
    unsigned int symbol, length, rhstype;

    *new_pos = pos;
    *rhs_type = PCFG_SYMBOL_TYPE;
    *rhs_symbol = NIL;
	...
	if (length != 0) /* length = 0 means sentinel symbol found */
	  {
	    ...
	    assert ((rhstype == PCFG_NONTERMINAL_TYPE) ||
		    (rhstype == PCFG_BOOLEAN_TYPE));
	    *rhs_type = rhstype;
...
{
    unsigned int pcfg_rule, p;

    if (Sentinel1_Symbol == NIL)
        Sentinel1_Symbol = TXT_sentinel1_symbol ();

    pcfg_rule = TXT_create_table (TLM_Dynamic, 0);
    if (pcfg_rule >= PCFG_labels_alloc)
      { /* extend array */
        PCFG_labels = (char **)
	    Realloc (130, PCFG_labels, (pcfg_rule + 1) * sizeof (char *),
		     PCFG_labels_alloc * sizeof (char *));

	/* Initialize all potentially new label pointers to NULL */
	for (p = PCFG_labels_alloc; p < pcfg_rule + 1; p++)
	    PCFG_labels [p] = NULL;

	PCFG_labels_alloc = pcfg_rule + 1;
      }
    PCFG_labels [pcfg_rule] = (char *)
        Malloc (131, (strlen (label) + 1) * sizeof (char));
    strcpy (PCFG_labels [pcfg_rule], label);

    return (pcfg_rule);
}

boolean
PCFG_valid_rule (unsigned int parent_rule, unsigned int pcfg_rule);
/* (Recursively) Checks if the PCFG rule is valid. */

boolean
PCFG_valid_rhs (unsigned int parent_rule, unsigned int pcfg_rule,
	       unsigned int rhs)
/* Returns TRUE if the rhs is valid. */
{
    boolean pcfg_rule_exists, pcfg_rule_valid;
    ...
    */
    ...
    for (;;)
      {
	if (!PCFG_get_rhs (rhs, rhs_pos, &rhs_type, &rhs_symbol, &new_rhs_pos))
	    break; /* no more symbols to process */

	switch (rhs_type)
	  {
	  case PCFG_NONTERMINAL_TYPE:
	    pcfg_rule_exists = TXT_valid_table (rhs_symbol);
	    assert (pcfg_rule_exists);

	    pcfg_rule_valid = (rhs_symbol != parent_rule) ||
	        (new_rhs_pos >= rhs_length);
	    assert (pcfg_rule_valid);

	    if ((rhs_symbol != parent_rule) && (rhs_symbol != pcfg_rule) &&
		!PCFG_valid_rule (parent_rule, rhs_symbol))
	        return (FALSE);
	    break;
	  case PCFG_BOOLEAN_TYPE:
	    break;
	  default:
	    assert ((rhs_type == PCFG_SYMBOL_TYPE) ||
		    (rhs_type == PCFG_NONTERMINAL_TYPE) ||
		    (rhs_type == PCFG_BOOLEAN_TYPE));
	    break;
	  }
	rhs_pos = new_rhs_pos;
      }
...
/* (Recursively) Checks if the rule is valid. */
{
    ...
    assert (TXT_valid_table (pcfg_rule));

    /*
      fprintf (stderr, "checking valid rule %d for parent rule %d\n",
               pcfg_rule, parent_rule);
    */

    TXT_reset_table (pcfg_rule);
    while (TXT_next_table (pcfg_rule, &rhs, &rhs_id, &rhs_count))
        if (!PCFG_valid_rhs (parent_rule, pcfg_rule, rhs))
	    return (FALSE);
    return (TRUE);
}

void
PCFG_define_rhs (unsigned int rhs_type, unsigned int rhs,
		 unsigned int rhs_symbol)
/* Inserts the rhs_symbol and its type into the rhs of the PCFG rule. */
{
...
*/
{
    ...
    assert (TXT_valid_table (pcfg_rule) && (pcfg_rule < PCFG_labels_alloc) &&
	    (PCFG_labels [pcfg_rule] != NULL));
	...
	switch (symbol)
	  ...
	  case 'r': /* indicates rule symbol follows */
	    ...
	    PCFG_define_rhs (PCFG_NONTERMINAL_TYPE, rhs, arg_table);
	    break;
	  case 'b': /* indicates special symbol follows; then
	    ...
	    PCFG_define_rhs (PCFG_BOOLEAN_TYPE, rhs, arg_function);
	    break;
	  case '$':
	    TXT_append_symbol (rhs, Sentinel1_Symbol);
    ...
    TXT_update_table (pcfg_rule, rhs, &rhs_id, &rhs_count);

    assert (PCFG_valid_rule (pcfg_rule, pcfg_rule));
}

void
PCFG_dump_rule1 (unsigned int file, unsigned int pcfg_rule);
/* (Recursively) Dumps the rule. */

void
PCFG_dump_rhs (unsigned int file, unsigned int pcfg_rule, unsigned int rhs)
/* (Recursively) Dumps the rhs of the rule. */
{
    ...
    fprintf (Files [file], "%s [%d] --> ", PCFG_labels [pcfg_rule], pcfg_rule);
	...
	switch (rhs_type)
	  ...
	  case PCFG_SYMBOL_TYPE:
	    ...
	    break;
	  case PCFG_NONTERMINAL_TYPE:
	    fprintf (Files [file], "<%s>[%d]", PCFG_labels [rhs_symbol],
		     rhs_symbol);
	    break;
	  case PCFG_BOOLEAN_TYPE:
	...
	if (!PCFG_get_rhs (rhs, rhs_pos, &rhs_type, &rhs_symbol, &new_rhs_pos))
	    break; /* no more symbols to process */
	if (rhs_type == PCFG_NONTERMINAL_TYPE)
	    PCFG_dump_rule1 (file, rhs_symbol);
	rhs_pos = new_rhs_pos;
      }
}
...
/* (Recursively) Dumps the rule. */
{
    ...
    if (BITS_ISSET (PCFG_set, pcfg_rule))
      return; /* already dumped this set */
    BITS_SET (PCFG_set, pcfg_rule);

    assert (TXT_valid_table (pcfg_rule));

    TXT_reset_table (pcfg_rule);
    while (TXT_next_table (pcfg_rule, &rhs, &rhs_id, &rhs_count))
        PCFG_dump_rhs (file, pcfg_rule, rhs);
}
...
/* Dumps the PCFG rule (for debugging purposes). */
{
    ...
    PCFG_dump_rule1 (file, pcfg_rule);
}

unsigned int PCFG_Parse_Cache = NIL;
unsigned int PCFG_Parse_Cache_Text = NIL;
unsigned int PCFG_Parse_Cache_Data = NIL;

void
PCFG_set_parse_cache_text (unsigned int pcfg_rule, unsigned int rhs,
			  unsigned int text_pos)
/* For setting the text to be inserted/searchd for in the parse cache. */
{
    TXT_setlength_text (PCFG_Parse_Cache_Text, 0);
    TXT_append_symbol (PCFG_Parse_Cache_Text, pcfg_rule);
    TXT_append_symbol (PCFG_Parse_Cache_Text, rhs);
    TXT_append_symbol (PCFG_Parse_Cache_Text, text_pos);
}

boolean
PCFG_parse_rule (unsigned int pcfg_rule, unsigned int level,
		unsigned int text, unsigned int text_pos,
		unsigned int *new_text_pos);
/* Parses the text using the PCFG rule. */

boolean
PCFG_parse_rhs (unsigned int pcfg_rule, unsigned int level,
	       unsigned int pcfg_rule_pos, unsigned int rhs,
	   unsigned int text, unsigned int text_pos,
	   unsigned int *new_text_pos)
/* Parses the text using the rhs (right hand side) of the PCFG rule. */
{
    unsigned int rhs_symbol, rhs_type, text_symbol;
    unsigned int old_text_pos, this_text_pos, rhs_pos, new_rhs_pos;
    unsigned int key_id, key_data, key_count;
    ...
    *new_text_pos = text_pos;
    ...
    /*
      ...
      PCFG_dump_rhs (stderr, pcfg_rule, rhs);
    */

    /* Check the cache first */
    PCFG_set_parse_cache_text (pcfg_rule, rhs, text_pos);
    if (TXT_getid_table (PCFG_Parse_Cache, PCFG_Parse_Cache_Text,
			 &key_id, &key_count))
      { /* found this (pcfg_rule, rhs, text_pos) tuple in the table */
	assert (TXT_get_index (PCFG_Parse_Cache_Data, key_id, &key_data));
	*new_text_pos = key_data; /* the key data contains the text pos */
	...
	switch (rhs_type)
	  ...
	  case PCFG_SYMBOL_TYPE:
	    ...
	    text_pos++;
	    break;
	  case PCFG_NONTERMINAL_TYPE: /* Recursively match the PCFG rule */
	    if (!PCFG_parse_rule (rhs_symbol, level+1, text, text_pos,
				 new_text_pos))
	        return (FALSE);
	    text_pos = *new_text_pos;
	    break;
	  case PCFG_BOOLEAN_TYPE: /* match the boolean function */
    ...
    *new_text_pos = text_pos;
    ...
    PCFG_set_parse_cache_text (pcfg_rule, rhs, old_text_pos);
    TXT_put_index (PCFG_Parse_Cache_Data, key_id, text_pos); /* *** This needs to be checked */

    return (TRUE); /* the rhs matches the text */
}

boolean
PCFG_parse_rulepos (unsigned int pcfg_rule, unsigned int level,
		   unsigned int pcfg_rule_pos, unsigned int pcfg_rule_tablepos,
		   unsigned int text, unsigned int text_pos,
		   unsigned int *new_text_pos)
/* Parses the text using the PCFG rule. */
{
    ...
    unsigned int expand_tablepos, new_pcfg_rule_pos;
    boolean (*boolean_function)  (unsigned int);
    ...
    *new_text_pos = text_pos;
    ...
    fprintf (stderr, "parsing rule %d rule_pos %d text pos %d\n",
	     pcfg_rule, pcfg_rule_pos, text_pos);
    */

    if ((pcfg_rule == NIL) || (pcfg_rule_tablepos == NIL))
        return (FALSE);
	...
	TXT_get_tablepos (pcfg_rule, pcfg_rule_tablepos, &rhs, &rhs_id,
			  &rhs_count, &rhs_symbol, &rhs_symbols);
	/*
	fprintf (stderr, "pcfg_rule = %d tablepos = %d (tnode ptr %p) rhs = %d rhs_id = %d rhs_count = %d rhs_symbol = ",
		 pcfg_rule, pcfg_rule_tablepos, (struct table_trie_type *)
		 pcfg_rule_tablepos, rhs, rhs_id, rhs_count);
	...
	*/
	...
	if (rhs_symbols == NIL)
	  {
	    rhs_type = PCFG_SYMBOL_TYPE;
	    new_pcfg_rule_pos = pcfg_rule_pos + 1;
	  }
	else
	  {
	    ...
	    new_pcfg_rule_pos = pcfg_rule_pos + 4;
	  }
	...
	switch (rhs_type)
	  ...
	  case PCFG_SYMBOL_TYPE:
	    ...
	    break;
	  case PCFG_NONTERMINAL_TYPE:
	    if (PCFG_parse_rule (rhs_symbol1, level+1, text, text_pos,
				&next_text_pos))
	        match_found = TRUE;
	    break;
	  case PCFG_BOOLEAN_TYPE:
	...
	if (match_found)
	  { /* symbols match */
	    rhs_ok = (rhs != NIL) &&
	      PCFG_parse_rhs (pcfg_rule, level, new_pcfg_rule_pos, rhs,
			     text, next_text_pos, &this_text_pos);
	    if (rhs_ok && (this_text_pos > greatest_text_pos))
	        greatest_text_pos = this_text_pos;
	    ok = ok || rhs_ok;

	    expand_tablepos = TXT_expand_tablepos (pcfg_rule, pcfg_rule_tablepos);
	    expand_ok = (expand_tablepos != NIL) && PCFG_parse_rulepos
	        (pcfg_rule, level, new_pcfg_rule_pos, expand_tablepos,
		 text, next_text_pos, &this_text_pos);
	...
	pcfg_rule_tablepos = TXT_next_tablepos (pcfg_rule, pcfg_rule_tablepos);

      }

    if (ok && (level == 0) && (pcfg_rule_pos == 0))
        fprintf (stderr, "*** pcfg_rule %d at pos %d new pos %d\n",
		 pcfg_rule, text_pos, greatest_text_pos);
...
/* Parses the text using the PCFG rule. */
{
    unsigned int pcfg_rule_tablepos;
    boolean ok;

    pcfg_rule_tablepos = TXT_reset_tablepos (pcfg_rule);
    ok = PCFG_parse_rulepos (pcfg_rule, level, 0, pcfg_rule_tablepos,
			    text, text_pos, new_text_pos);
    return (ok);
}
   ...
   result in a new text record. */
{
    ...
    PCFG_Parse_Cache = TXT_create_table (TLM_Dynamic, 0);
    PCFG_Parse_Cache_Text = TXT_create_text ();

    textlen = TXT_length_text (text);
    for (pos = 0; pos < textlen; pos++)
      {
	start_pos = pos;
	PCFG_parse_rule (pcfg_rule, 0, text, start_pos, &end_pos);
	...
	*/
    ...
    TXT_release_table (PCFG_Parse_Cache); 
    TXT_release_text (PCFG_Parse_Cache_Text);
